...
const migrationCb = (migrationData) => {
  migrationData.startTime = now();
  console.log("Migration Start Time", migrationData.startTime);
...
const startHandler = (payload, programStartTime) => {
  ...
  if (migrationData.startTime) {
    ...
    fs.appendFileSync(
      ...
      },${BigInt(migrationData.endTime) - BigInt(migrationData.startTime)},${
        payload.length
      }`
    );
  }
