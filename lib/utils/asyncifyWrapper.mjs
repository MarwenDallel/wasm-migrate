...
export class AsyncifyData {
  ...
  static fromBinary(dump, baseAddr, memory) {
    ...
    // Temporary workaround as I am unable to load the dump from the binary.
    // TODO: Fix this.
    const filePath = `dump/temp${timestamp()}.bin`;
    fs.writeFileSync(filePath, dump);

    return AsyncifyData.fromFile(filePath, baseAddr, memory);
  }
