...
function initiate(wasmFile, startFnName, optimize, args) {
  const filename = path.parse(wasmFile).name; // strip extension
...
export function run(wasmFile, startFnName, args, optimize, port) {
...
export function resume(wasmFile, dumpFile, startFnName, args, port) {
  const { startFnCallback, instance } = initiate(
    wasmFile,
    startFnName,
    false,
  ...
  wrapper = new AsyncifyWrapper(startFnCallback, instance.exports, data, true);
